...
Here git add prepare the what changes will be included in the next commit, while git commit finalizes those changes and saves them in the repository history.


#### git add – Stage changes
Purpose:

To prepare specific changes (modified files) to be included in the next commit.It moves changes from the working directory ➝ to the staging area.Think of it as selecting what you want to save.

#### git commit – Save staged changes
Purpose:
To permanently record the staged changes into the project’s history.It moves changes from the staging area ➝ to the repository (the actual Git history).Each commit has a message and a snapshot of what was staged.



### 7. Git add 
Git add is used to move changes from the working directory to the staging area. This prepares the changes to be committed later. You can add specific files or all changes. You can add specific files or all changes.
```bash
...
> To add and commit changes in one step, you can use:
```bash
...
### 8.Git Remote
Since Git is often used for collaboration, you can connect your local repository to a remote repository (like GitHub or GitLab) to share changes with others.so to connect your local repository to a remote repository, you can use git remote commands.
```bash
git remote -v
```
...
```
<alias> is a short name for the remote repository
 and <url> is the URL of the remote repository.
...
```
when we put `-u` flag, it sets the upstream branch for the current branch, allowing you to use `git push` without specifying the remote and branch in future pushes and git pull will automatically know which remote branch to pull from as
```bash
git push
git pull
```
...
Example:
Suppose you cloned a remote repository. Meanwhile, a collaborator also cloned the same repository, made some changes, and pushed them to the remote branch.By running git fetch, you download those remote changes. However, your current branch remains unchanged until you explicitly merge those changes.
...
This command applies the changes fetched from the remote branch (`origin/main`) into your local branch, updating your working copy with the latest changes.Such that my local branch now contains both your changes and the updates made by your collaborator.
...
![Git Branching Example](/images/git.png)
...
## 16 Combining Branch 
## 16.1 Git Merge
To merge changes from one branch into another. Ie combining the work done in different branches into a single branch.
...
### 16.4 Example of Merging Branches
Suppose i have master branch and a feature branch called `feature-xyz`.and i change in both branches and now i want to combine the changes from `feature-xyz` into `master`.
...
> Note: This command work only where there is merge conflict, if there is no merge conflict, it will not work.
...
>> Which means it can be used between git merge and git commit, if you want to stop the merge process and return to the state before the merge started.
...
## 18. Going Back in History
As git is a version control system, it allows you to go back in history and view or revert to previous versions of your files.To go back to the previous commit as:

### 1. Git reset
This command will allow to go back to the previous commit and discard all changes made after that commit.
```bash
...
A-> B -> C -> D (HEAD)
```
If you run `git revert D`, it will create a new commit that undoes the changes made in commit C, and the history will look like this:
...
similarly we we want to undo the change made in one of the previous commits, we can use `git revert` with the commit hash of that commit.
...
> Note: It only undoes the change made in that commint only not before commit and after commit

So git reset --hard <commit A hash> is equivalent to gir revert <commit D hash> git revert <commit C hash> git revert <commit B hash> .If we do only git revert <commit B  hash> it will only undo the changes made in commit B and not the changes made in commit C and D.



why to use git commit instead of git reset 
...
git config --list                               # Display the current Git
```
